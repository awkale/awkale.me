...
| `docs/archive/program-*-liyo-tilles-center.pdf` | Three more scanned LIYO programs, added 2026-08-30, all at Tilles Center, Long Island University C.W. Post Center, Brookville: **1992-12-13** (30th anniversary, first concert of 1992-93 — Dreiwitz, Susan Deaver associate), **1993-05-02** (third program of the same season, same pair) and **1995-06-11** (32nd season, fourth concert of 1994/95 — Dreiwitz, Scott Stickley associate, with soloists Seth Abrams, Terrence Fay, Jessica Hull and Dickran Kazanjian). **None of the three is in the archive** — not in Contentful, not in the xlsx, not in `bso-graph.json` — so like the 1993-07-26 program they are primary sources for concerts no other source here holds, and the space holds only two LIYO concerts until they are transcribed. Two pages each, image-only scans with **no text layer**, so reading one means rendering it. They corroborate CONTEXT.md's LIYO numbering from a second direction: Season 30 is 1992-93 and Season 32 is 1994-95. **Recompressed on the way in, from ~32 MB to 2.4 MB.** The scanner emitted them as *lossless* RGB bitmaps, which is the whole reason two pages cost 10 MB; re-encoding each page as JPEG at 2200px keeps the typewritten text sharp — verified by rendering, not assumed — at a twelfth the size. Do the same to any scan added here, and check legibility by eye rather than by file size. |
