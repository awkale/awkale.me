...
There is no build. This repo currently holds the token scaffolding and page
structures from AWK-22 — no dependencies installed, so the `.tsx` files do not
typecheck yet. `README.md` has the exact commands to wire it up, and
`app/data/sample.ts` is placeholder because none of the decided Contentful schema
exists in the space.
