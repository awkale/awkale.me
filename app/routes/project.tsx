    ...
    <main className="px-[var(--gutter)] py-[var(--space-section)]">
      <article className="mx-auto max-w-[var(--width-content)]">
        <header>
          ...
          <h1 className="font-display mt-2 text-3xl font-semibold tracking-tight">{project.title}</h1>
          ...
          <dl className="mt-5 flex flex-wrap gap-8 border-t border-border-subtle pt-4">
            <div>
              <dt className="text-[0.68rem] uppercase tracking-[0.1em] text-muted-foreground">Years</dt>
              <dd className="tabular mt-0.5 text-sm">{project.years}</dd>
            </div>
            <div>
              <dt className="text-[0.68rem] uppercase tracking-[0.1em] text-muted-foreground">Live</dt>
            ...
            <div>
              <dt className="text-[0.68rem] uppercase tracking-[0.1em] text-muted-foreground">
                Technologies
              </dt>
        ...
        <div className="typeset typeset-reading mt-10 max-w-[var(--measure)]">
          ...
          <p>
            ...
            describing them. The interesting part of the work was not building a fifth button. It
            was finding out why the previous four had happened, and making the fifth one the path of
            least resistance.
          </p>
          <p>
            That reframing is what turned a component library into a design system. A library is a
            folder of components. A system is a set of decisions about which components may exist,
            expressed so that following them is easier than not.
          </p>

          <h3>Tokens came before components</h3>
          <p>
            The first shipped artifact was not a component at all — it was a three-layer token
          ...
          <blockquote>
            A token nobody uses is worse than a missing one, because it looks like something depends
            on it.
          </blockquote>
          ...
          <figure className="m-0 grid gap-4 md:grid-cols-2">
            <div
              className="aspect-[4/3] rounded-[var(--radius)] border border-border-subtle
                         bg-[repeating-linear-gradient(0deg,var(--muted)_0_8px,var(--card)_8px_16px)]"
              role="img"
              aria-label="Before: four divergent button implementations"
            />
            <div
              className="aspect-[4/3] rounded-[var(--radius)] border border-border-subtle
                         bg-[linear-gradient(135deg,var(--primary),var(--muted)_70%)]"
              role="img"
          ...
          <p>
            The tables were the hard part. AG Grid brings its own opinions about layout, density and
            focus, and a loan-analytics product is mostly tables. The pattern we landed on wraps AG
            Grid rather than restyling it, so a consumer changing one column&rsquo;s behaviour cannot
            change every other consumer&rsquo;s.
          </p>
        </div>
      </article>
    </main>
