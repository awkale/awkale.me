    ...
    <main className="px-[var(--gutter)] py-[var(--space-section)]">
      <div className="mx-auto max-w-[var(--width-wide)]">
        ...
        <p className="mt-1 max-w-[56ch] text-sm text-muted-foreground">
          Two carry case studies. Three are index-only.
        </p>
          ...
          <tbody>
            {PROJECTS.map((p) => (
              <tr key={p.slug} className="hover:bg-muted">
                ...
                <Td className="text-[0.9em] text-muted-foreground">
                  {p.hasBody ? "case study" : "—"}
                </Td>
              </tr>
            ))}
          </tbody>
...
function Th({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <th
      className={`border-b border-border px-2 py-1.5 text-left text-[0.66rem] font-medium
                  uppercase tracking-[0.09em] text-muted-foreground ${className}`}
    >
      {children}
    </th>
  );
