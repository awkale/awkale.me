...
type Schema = {
  types: { id: string; addFields: Field[]; helpText?: Record<string, string> }[]
...
describe('archive-schema.json', () => {
  ...
  it('puts no explanation in a field name', () => {
    ...
    //
    // That home is help text, which this file now DECLARES: a `helpText` map on
    // each type group, reconciled against the editor interface by
    // migrate_schema.py (AWK-62). Both halves of the convention are checkable
    // now — this asserts the name, and the `help text` block below asserts the
    // explanation.
  ...
  describe('help text — AWK-62', () => {
    // Help text is the one thing this file declares that does NOT live on the
    // content type: Contentful keeps it on the editor interface, which the
    // Delivery API never serves. So it is also the one thing migrate_schema.py
    // CORRECTS rather than reports — the file is its source of truth.
    //
    // The map is keyed by field id at the GROUP level, beside `addFields`,
    // because three of the six fields are not in `addFields` at all: work.slug
    // is repurposed in place, and programItem.composer and programItem.credits
    // predate this pipeline entirely.
    const declared = schema.types.flatMap((t) =>
      Object.entries(t.helpText ?? {}).map(([fieldId, text]) => [`${t.id}.${fieldId}`, text] as const)
    )

    it('declares the six texts the space carries, and no others', () => {
      // Swept live on 2026-08-30 and again on 2026-09-08: six fields carry help
      // text, every one typed by hand in the web app. A seventh appearing here
      // is a text nobody has written yet — which the applier would CREATE on
      // its next run, against a space nobody asked to change.
      expect(declared.map(([id]) => id).sort()).toEqual(
        [
          'composer.slug',
          'programItem.composer',
          'programItem.conductor',
          'programItem.credits',
          'season.orchestras',
          'work.slug',
    ...
    it('gives every one of them a real text', () => {
      // Non-vacuity, and a real guard: setting a text to empty is not in scope
      // for the applier, so an empty string here would declare a state it
      // cannot reach and would rewrite on every run.
      expect(declared.length).toBe(6)
      for (const [id, text] of declared) {
        expect(typeof text, `${id} needs a string`).toBe('string')
        expect(text.trim(), `${id} needs a real help text`).not.toBe('')
      }
    })

    it('tells the editor that an empty programItem.conductor means inherit', () => {
      // The reason this ticket exists. `programItem.conductor` is optional and
      // normally empty, and EMPTY MEANS SOMETHING — the item takes the
      // Concert's Conductor, resolved in app/lib/archive.ts. An empty reference
      // field carrying a meaning is invisible in the web app, so this sentence
      // is the only place the person entering data can read the rule.
      const text = schema.types.find((t) => t.id === 'programItem')?.helpText?.conductor ?? ''

      expect(text).toMatch(/empty means/i)
      expect(text).toMatch(/inherits the Concert's Conductor/)
    })

    it('tells the editor not to type a slug by hand', () => {
      // Both slug texts end the same way, and the ending is the load-bearing
      // half: slugs are GENERATED by backfill_slugs.py (ADR-0008), and the
      // alternative to that sentence is 600 hand-typed addresses.
      for (const type of ['work', 'composer']) {
        const text = schema.types.find((t) => t.id === type)?.helpText?.slug ?? ''

        expect(text, `${type}.slug`).toMatch(/do not type one\.$/)
        expect(text, `${type}.slug`).toContain('scripts/contentful/backfill_slugs.py')
      }
    })

    it('keeps helpText out of every field object', () => {
      // A field object is sent to the Management API VERBATIM. An unknown key
      // inside one is refused or dropped — and worse, `_differing_keys()`
      // compares every key the spec sets against the live field, and a content
      // type never holds `helpText`, so the field would read as drift forever.
      // That is why the declaration is a group-level map instead.
      const fields = [...schema.types.flatMap((t) => t.addFields), ...schema.pinnedFields.flatMap((t) => t.fields)]

      expect(fields.length).toBeGreaterThan(0)
      for (const f of fields) {
        expect(Object.keys(f), `${f.id} must stay a verbatim Contentful field object`).not.toContain('helpText')
      }
    })
  })
